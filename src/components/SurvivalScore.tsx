...
const EMP = [
  ...
  ["gov", "emp_gov"],
  ["business", "emp_business"],
  ["self", "emp_self"],
  ["freelance", "emp_freelance"],
];
const COL = [
  ["re", "colt_re"],
  ["vehicle", "colt_vehicle"],
  ["savings", "colt_savings"],
  ["none", "colt_none"],
];
...
export default function SurvivalScore() {
  ...
  const pkgsForPurpose = useMemo(
    () =>
      PKG.map((p, i) => ({ ...p, idx: i })).filter(
        (p) => p.purpose === f.purpose,
      ),
    [f.purpose],
  );
      ...
      <div className="wrap">
        ...
        <div className="surv-grid">
            ...
            <div className="fgroup">
                ...
                >
                  ...
                  {pkgsForPurpose.map((p) => (
                    <option key={p.idx} value={p.idx}>
                      {bankOf(p.code).name} · {prodName(p, lang)}
                    </option>
                  ))}
                </select>
              </label>
              {activePkg && (
                <div className="pkg-policy">
                  {t("pkg_max")}: {(activePkg.max / 1e9).toFixed(1)}B VND ·{" "}
                  {t("pkg_term")}: {activePkg.term} {t("r_mo")} · LTV:{" "}
                  {activePkg.ltv}%
                </div>
              )}
          ...
          <div className="surv-result">
            {res ? (
              <Result res={res} lang={lang} t={t} />
            ) : (
              <div className="surv-empty">{t("surv_empty")}</div>
            )}
          </div>
        </div>
      </div>
...
function Result({
  ...
  res: { mc: MCResult; T: number; inp: SurvInput };
  lang: Lang;
  t: (k: string) => string;
}) {
  const { mc, T, inp } = res;
  ...
  const recos: [string, string][] = [
    ...
    [
      t("reco_pkg"),
      bestP ? bankOf(bestP.code).name + " · " + prodName(bestP, lang) : "—",
    ],
  ];
  if (
    selP &&
    cheapest &&
    cheapest.code !== selP.code &&
    cheapest.rate < selP.rate
  ) {
    recos.push([
      t("reco_alt"),
      bankOf(cheapest.code).name +
        " · " +
        prodName(cheapest, lang) +
        " (" +
        cheapest.rate +
        "%)",
    ]);
    ...
    <>
      {selP && (
        <div className="surv-for">
          ...
          <div>
            <span>{t("surv_for")}</span>
            <b>
              {bankOf(selP.code).name} · {prodName(selP, lang)}
            </b>
          </div>
          <span className="sf-rate">
            {selP.rate}% → {selP.std}%
          </span>
        </div>
      )}
      ...
      <div className="surv-chart">
        <SurvivalChart mc={mc} T={T} t={t} />
      </div>
      <div className="surv-legend">
        <span className="lg lg-med">{t("r_median")}</span>
        <span className="lg lg-band">{t("r_band")}</span>
        <span className="lg lg-stress">{t("r_stress")}</span>
      </div>
      <div className="rc-clab">{t("r_reco")}</div>
      <div className="reco-list">
        {recos.map((r, i) => (
          <div className="reco-i" key={i}>
            <span className="reco-k">{r[0]}</span>
            <span className="reco-v">{r[1]}</span>
          </div>
        ))}
      </div>
      ...
      <div className="sc-explain">
        ...
        {met.efr < 3 && (
          <p className="sc-reason">
            <b>
              {t("m_efr")} = {met.efr.toFixed(1)} {t("r_mo")}
            </b>{" "}
            — {t("why_efr")}
          </p>
        )}
        {met.disposable < 0 && (
          <p className="sc-reason">
            <b>
              {t("m_disp")} = {fmtMonthly(met.disposable)}
            </b>{" "}
            — {t("why_neg")}
          </p>
        )}
